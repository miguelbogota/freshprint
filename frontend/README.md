...
`UpdateStore` owns loading, selection, and decision state. `DecisionGateway` is the only HTTP boundary. `UpdateList` and `UpdateDetail` handle display and user intent. The fixture remains for isolated UI tests; the running app gets its data from the server. Components own their SCSS; the small `src/styles.scss` contains only document-wide defaults and reduced-motion behavior.
...
Tests cover the root shell, lazy routes, dashboard, list, detail, store, and HTTP gateway. Type declarations, barrel exports, and static fixture data are exercised through those behavior tests rather than given empty one-to-one spec files.
