...
import type { EngagementUpdate, UpdateDecision } from '../../data-access';

/** Review boundary: render server wording and emit Apply or Decline intent. */
@Component({
  selector: 'app-update-detail',
  styleUrl: './update-detail.component.scss',
  template: `
    <section class="panel detail" aria-label="Selected engagement">
      <div class="detail-top">
        <div>
          ...
          <h2>{{ item().name }}</h2>
          <p class="detail-sub">{{ item().template.displayName }}</p>
        </div>
        <span
          class="pill"
          [class.pending]="item().status === 'PENDING' && !item().declined"
          [class.current]="item().status === 'CURRENT'"
          [class.declined]="item().declined"
          [class.unknown]="item().status === 'UNKNOWN'"
          >{{
            item().declined
              ? 'Declined'
              : item().status === 'PENDING'
      ...
      @if (item().status === 'PENDING') {
        <div class="version-track">
          <span
            >Current version<strong>v{{ item().baselineVersion }}</strong></span
          ><span class="version-arrow">→</span
          ><span
            >Latest template<strong>v{{ item().targetVersion }}</strong></span
          ...
          @switch (summary.state) {
            @case ('AVAILABLE') {
              <div class="section-title">
                <h3>What changed</h3>
                <span>From your version to the latest</span>
              </div>
                ...
                <div class="change-group">
                  ...
                  @for (change of group.changes; track $index) {
                    <div class="change-row">
                      <span
                        class="change-dot"
                        [class.added]="change.kind === 'ADDED'"
                        [class.changed]="change.kind === 'CHANGED'"
                        [class.removed]="change.kind === 'REMOVED'"
                        >{{
                      ...
                      <p>
                        ...
                        @if (change.reviewRecommended) {
                          <span class="review-note">Review carefully.</span>
                        }
                      </p>
                    </div>
                  }
            ...
            @case ('UNAVAILABLE') {
              <p class="callout warning">
                Change summary is unavailable right now. Please check again later.
              </p>
            }
          }
        } @else {
        ...
        @if (item().declined) {
          <p class="callout">
            You declined this version. Another published version will make a new review available.
          </p>
        }
        <div class="actions">
          <button
        ...
        <p class="callout warning">
          Update information is not available yet. Please check again later.
        </p>
      }
    </section>
  `,
