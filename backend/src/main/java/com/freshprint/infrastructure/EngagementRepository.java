...
public class EngagementRepository {
  ...
  public Optional<EngagementBaseline> find(String id) {
    return jdbc.query("SELECT engagement_id, name, template_id, template_version FROM engagements WHERE engagement_id = ?",
        (rs, row) -> new EngagementBaseline(rs.getString(1), rs.getString(2),
            rs.getString(3), rs.getInt(4)), id).stream().findFirst();
  }
  ...
  ApplicationRunner seedEngagements(JdbcTemplate jdbc, ObjectMapper mapper) {
    return args -> {
      ...
      if (jdbc.queryForObject("SELECT COUNT(*) FROM engagements", Integer.class) != 0) return;
      try (var input = new ClassPathResource("fixtures/engagements.json").getInputStream()) {
        for (var node : mapper.readTree(input)) {
          jdbc.update("INSERT INTO engagements (engagement_id, name, template_id, template_version) VALUES (?, ?, ?, ?)",
